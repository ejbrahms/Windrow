...
// real Wispfield identity, upserts it into the store, and prints `{identity, principalId}` as
...
// loom process that spawned it.
//
// Prints `{"identity": null, "principalId": null}` and exits 0 (not an error) when run outside
// Wispfield (no LOOM_NODE_ID) — a fail-open no-identity case the caller decides how to handle.
