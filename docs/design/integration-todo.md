# TODO: wiring the capability-governance system to real Wispfield data

> [!warning]
> Nothing observed by the running system today is real. Capabilities match this environment's
> actual skills/MCPs, and instance principals now map to this field's real loom roster (name,
> agent type/backend, field — see item 2), but every grant and usage event is still synthetic,
...
| 2 | Map real principals | ✅ Done — see "Principal mapping (v1)" in `docs/design/api-contract.md`. `server/principals/` pulls real loom name/agent type/backend/field from Wispfield's own env vars into instance principals, id scheme is role-per-agentType + instance-per-loom-id. Still open: nothing yet marks an instance principal `stale` when its loom leaves the field. | — |
...
| 4 | Correlation ids | ✅ Done — `correlationId` is now `<loom name>(<loom node id>):<session id>` (see `server/hooks/pre-tool-use.js`). Wispfield doesn't expose a dispatched-task id to a loom's child processes, so the session id (the task/turn boundary within that loom) is the closest real proxy; revisit if Wispfield ever surfaces a true task id to hooks. | 3 |
...
prompt is what Wispfield already renders on the loom's card, so the human sees and answers it in
the same place they're already watching the loom, with no separate dashboard tab or push
