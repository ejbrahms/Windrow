...
description: Use when asked to open, show, or check the capability-governance dashboard (capabilities, grants, principals, usage) — brings it up as a live card instead of a separate browser tab.
---

# Open capabilities dashboard

This project has no true native panel inside the host platform's own app shell (that's
closed-source, not this repo) — the closest native integration is a **live card**: a browser
agent instance pointed at the governance client, sitting alongside every other agent instead of a
separate browser tab. This skill gets one on screen with the fewest steps, reusing an existing
...
Check the workspace for a browser agent already titled `Capabilities` (`wispfield_get_field_status`).
